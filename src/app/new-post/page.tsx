'use client'
import { useState, useRef, useMemo } from 'react';
import Editor from '@/components/Editor';
import axios from 'axios';

export default function Home () {
    const [title, setTitle] = useState('');
    const [content, setContent] = useState('');

    const categoryList = ['Frameworks', 'Programming', 'Tools', 'Software Engineering', 'Interview Preparation'];
    const [category, setCategory] = useState(categoryList[0]);

    const handleCategoryClick = (category:string) => {
        setCategory(category);
    };

    const handlePublish = ()=>{
        if(!title){
            alert('Enter a title')
            return
        }
        if(!content){
            alert('Enter a content')
            return
        }
        
        const addBlogBody ={
            title: title,
            body: content,
            createdBy: 'User1',
            category: category
        }

        axios.post('http://localhost:3002/blogs/add', addBlogBody , {
                headers: {
                    'Content-Type': 'application/json'
                }
            })
            .then(response => {
                console.log('Response:', response.data);
            })
            .catch(error => {
                console.error('Error:', error);
            });
    }

    return (
        <div className='flex flex-col justify-center'>
            <div className='flex'>
                <div className='w-1/6'></div>
                <div className='flex w-5/6'>
                    <input 
                        id='title' 
                        placeholder='Enter Title' 
                        value={title} 
                        className='border-b my-4 w-4/5 text-4xl' 
                        onChange={(e)=>setTitle(e.target.value)}>
                    </input>
                    <button 
                        type="submit" 
                        className="w-1/10 h-1/2 m-4 rounded bg-green-600 active:bg-green-700 p-2 text-sm text-white shadow-md shadow-gray-400"
                        onClick={handlePublish}>
                        Publish
                    </button>
                </div>
            </div>
            <div className='flex'>
                <div className='w-1/6 pl-2'>
                    <p className='text-xl text-gray-500'>Choose Category...</p>
                    <select id="categoryDropdown" className='border border-gray-600 rounded-md' onChange={(e)=>setCategory(e.target.value[0])}>
                        {
                            categoryList.map((category,index) => (
                                    <option key = {index} value={category}>{category}</option>
                                ))
                        }
                    </select>
                </div>
                <Editor content={content} setContent={setContent}/>
            </div>
        </div>
    );
};